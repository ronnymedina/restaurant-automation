...
The platform is an **open-source, cloud-only SaaS**. The Electron desktop app (`apps/desktop`) still exists in the tree but is **inactive/unmaintained** — its former offline-distribution stack (license server, code obfuscation, protected binary builds) was removed.

### Repository structure
There is **no root workspace** — no root `package.json`, no pnpm workspace, no Turborepo. Each app is self-contained: it has its own `package.json`, lockfile, and Dockerfile, and is **built independently with Docker** from its own directory (`context: ./apps/<app>`). The two active apps are `apps/api-core` and `apps/ui`.
...
Both apps use multi-stage Dockerfiles and build **standalone from their own directory** (`context: ./apps/api-core` and `./apps/ui` in `docker-compose.yml`) — no root context, no workspace. `docker-compose.yml` at root uses the `dev` stage; Railway deploys use the `prod` stage.
