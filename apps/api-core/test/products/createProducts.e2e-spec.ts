...
describe('Products (e2e)', () => {
  ...
  describe('POST /v1/products', () => {
    ...
    it('Valida estrictamente las propiedades expuestas (seguridad opt-in) al crear', async () => {
      const res = await request(app.getHttpServer())
        .post('/v1/products')
        .set('Authorization', `Bearer ${adminTokenA}`)
        .send({ ...validPayload(categoryIdA), name: 'Creación Estricta', sku: 'STRICT-001' })
        .expect(201);
  ...
  describe('GET /v1/products/:id', () => {
    ...
    it('Valida estrictamente las propiedades expuestas (seguridad opt-in) al obtener por ID', async () => {
      const res = await request(app.getHttpServer())
        .get(`/v1/products/${createdProductId}`)
        .set('Authorization', `Bearer ${adminTokenA}`)
        .expect(200);
